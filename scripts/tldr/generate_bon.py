...
try:
    for idx, item in enumerate(tqdm.tqdm(dataset.select(range(start_idx, len(dataset))), initial=start_idx)):
        ...
        batch_size = script_args.gen.k  # per search call
        num_batches = (script_args.N + batch_size - 1) // batch_size

        all_outputs = []

        # Step 1: generate N candidates in batches
        for _ in range(num_batches):
            ...
            outputs = outputs.squeeze(1)  # shape: [batch_size, seq_len]
