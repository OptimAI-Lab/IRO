...
from src.inference_time_alignment.decoding import *
...
class CBSPosthocGenerationMixin(GenerationMixin):
    ...
    def search(
        ...
        decoding_strategy: Optional[int] = 0,
        ...
        **model_kwargs,
        ...
        while not this_peer_finished:
            ...
            next_token_logprobs = torch.log(
                probs.gather(1, next_tokens.unsqueeze(1)).squeeze(1)
            )  # shape: (batch_size,)
            ...
            #------------------------------ state selection ------------------------------#
            #-----------------------------------------------------------------------------#
            if (tokens_remain_per_chunk <= 0 or this_peer_finished == True) and w*k > 1 and scorer is not None:
                tokens_remain_per_chunk = l
                ...
                beam_scores = beam_scores.to(unfinished_sequences.device)
                ...
                elif decoding_strategy == 1:
                    beam_idx = get_diverse_beams_1(responses, beam_scores, unfinished_sequences, w, k)
                elif decoding_strategy == 2:
                    beam_idx = get_group_beams(responses, beam_scores, unfinished_sequences, w, k)
                elif decoding_strategy == 3:
                    beam_idx = get_beams_probability(responses, beam_scores, unfinished_sequences, w, k, temperature=selection_temperature)
                elif decoding_strategy == 4:
                    _, beam_idx = torch.topk(beam_scores+next_token_logprobs, w, dim=0, largest=True, sorted=True)
                    beam_idx = beam_idx.repeat(k) # repeat beam_idx by n. successors
